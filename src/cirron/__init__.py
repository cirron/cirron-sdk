...
# Without this eager import, the first ``Cirron.inference()`` call lazy-imports
# ``cirron.inference.decorator``, which rebinds ``cirron.inference`` to the
# submodule and shadows the module-level ``inference()`` function below.
...
def profile(
    ...
    Args:
        config: Inline profiling-section dict. Highest-precedence layer below
            explicit kwargs. Same shape as the ``profiling:`` block in
            ``cirron.yaml``.
        frameworks: Subset of frameworks to instrument (e.g. ``["torch"]``). When
            ``None``, all autodetected frameworks install hooks.
        snapshots: Snapshot policy. ``"stats"`` records per-tensor
            mean/std/min/max/norm inline at every epoch boundary; ``"sampled"``
            rolls ``random < sample_rate`` and serializes full tensors on a hit;
            ``"full"`` serializes every epoch (debug only).
        sample_rate: Probability per epoch boundary that a ``"sampled"`` capture
            fires. Ignored under ``"stats"`` / ``"full"``.
        flush_interval: Seconds between background spool flushes.
        enabled: When ``False``, returns a no-op ``Profiler`` and installs no
            hooks. Used by tests / CI to disable instrumentation without touching
            call sites.
        path: Override the ``cirron.yaml`` discovery path. When ``None``, walks up
            from cwd.
        output: Sink selection. ``"spool"`` (the default), ``"log"``,
            ``"stdout"``, ``"none"``, or a list of those. Defaults to the
            ``Cirron`` instance's ``output``, which is itself ``"spool"`` when
            unset. Sinks are local only; the platform transport is selected
            independently and is unaffected by this value.

    Returns:
        Profiler: The shared profiler singleton bound to the default ``Cirron``.
    """
...
def scope(name: str, index: int | None = None, **attrs: Any) -> Any:
    ...
    Args:
        name: Span name (e.g. ``"forward"``, ``"data_load"``). Shows up as the span
            label in the trace tree and platform UI.
        index: Optional positional index (e.g. epoch / batch number). Used by
            ``ci.epochs`` / ``ci.batches`` so the platform can sequence repeated
            spans of the same name.
        **attrs (Any): Arbitrary key/value metadata to attach to the span. Reaches
            the spool as ``span.attrs[key] = value``.

    Returns:
        Any: A context-manager object whose ``__enter__`` returns the ``Scope``
            instance.
    """
...
def mark(name: str, value: float | int | str | bool, **attrs: Any) -> None:
    ...
    Args:
        ...
        value: Metric value. Numeric values are preferred for downstream
            aggregation; strings / booleans are allowed for categorical /
            flag-style metrics.
...
def epochs(iterable: Iterable[Any]) -> Iterator[Any]:
    ...
    Args:
        iterable: Any iterable (typically ``range(n_epochs)``,
            ``enumerate(dataset)``, or a custom epoch-yielding generator).

    Yields:
        Any: Each item from ``iterable`` unchanged. The wrapper opens a ``epoch``
            scope before yielding and closes it after the consumer's loop body
            returns.
    """
...
def batches(iterable: Iterable[Any]) -> Iterator[Any]:
    ...
    Args:
        iterable: A batch-yielding iterable, typically a DataLoader or a sliced
            training set.
...
def secret(name: str) -> str:
    ...
    Raises:
        CirronSecretNotFound: If the name is invalid, no environment variable matches,
            the mount file is missing, or it is present but unreadable.
    """
...
def inference(
    ...
    *,
    ...
    Args:
        fn: The inference function. Populated automatically by the bare-decorator
            form.
        config: Optional per-call configuration: currently a forward-compat
            placeholder; the LLM helper in ``cirron.inference.llm`` reads
            provider / model / token-counter overrides from here.

    Returns:
        Callable[..., Any]: The wrapped function. When ``fn`` is ``None``, returns
            a decorator awaiting ``fn``.
    """
...
def wrap(estimator: Any) -> Any:
    ...
    Args:
        estimator (Any): An sklearn estimator or pipeline. Other objects are
            returned as-is.

    Returns:
        Any: A scope-aware proxy around ``estimator``, or ``estimator`` unchanged
            when no instrumentation applies.
    """
