...
class NumpyAdapter(DataAdapter):
    def __init__(self, data: Any, column_names: Optional[List[str]] = None) -> None:
        super().__init__(data)
        if data.ndim == 1:
...
class PolarsAdapter(DataAdapter):
    ...
    def get_dtypes(self) -> Dict[str, str]:
        return {col: str(dt) for col, dt in zip(self.data.columns, self.data.dtypes)}
    ...
    def get_numeric_columns(self) -> List[str]:
        ...
        return [c for c, dt in zip(self.data.columns, self.data.dtypes) if dt in numeric_types]

    def get_categorical_columns(self) -> List[str]:
        import polars as pl

        categorical = [pl.Utf8, pl.Categorical]
        return [c for c, dt in zip(self.data.columns, self.data.dtypes) if dt in categorical]
...
class ArrowAdapter(DataAdapter):
    ...
    def get_numeric_columns(self) -> List[str]:
        ...
        return [
            c for c in self.data.column_names
