...
class APIModelGenerator(BaseModelGenerator):
    ...
    def __init__(self, config: ModelConfig, api_endpoint: Optional[str] = None):
        ...
        if self.api_endpoint.startswith("http://"):
            logger.warning(
                "Using HTTP for API endpoint may expose data in transit. Consider using HTTPS for secure communication."
            )
    ...
    def _call_api(self, config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = requests.post(
                self.api_endpoint, json=config, headers=headers, timeout=30
            )
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    curl_command, capture_output=True, text=True, timeout=30
                )
    ...
    def _generate_code_locally(self, config: Dict[str, Any]) -> str:
        ...
        if framework in ["tensorflow", "tf", "keras"]:
            ...
            logger.warning(
                f"Unsupported framework {framework}, defaulting to tensorflow"
            )
            return self._generate_tensorflow_code(config, layers)
    ...
    def _generate_pytorch_code(self, config: Dict[str, Any], layers: list) -> str:
        ...
        for i, layer in enumerate(layers):
            ...
            if layer_type == "LINEAR" or layer_type == "DENSE":
                ...
                code_lines.append(
                    f"        self.layer_{i} = nn.Linear({in_features}, {out_features})"
                )
            elif layer_type == "LSTM":
                input_size = layer.get("input_size", 10)
                hidden_size = layer.get("units", 64)
                code_lines.append(
                    f"        self.layer_{i} = nn.LSTM({input_size}, {hidden_size}, batch_first=True)"
                )
    ...
    def _execute_generated_code(self, code: str) -> Any:
        ...
        try:
            ...
            for keyword in unsafe_keywords:
                if keyword in code:
                    raise Exception(
                        f"Unsafe code detected: '{keyword}' found in generated code"
                    )
            ...
            if "generated_model" in namespace:
                ...
                raise Exception(
                    "Generated code did not create 'generated_model' variable"
                )
        ...
        if self._generated_code:
            summary_lines.extend(
                [
                    ...
                    "-" * 30,
                    self._generated_code[:500]
                    + ("..." if len(self._generated_code) > 500 else ""),
                ]
            )
