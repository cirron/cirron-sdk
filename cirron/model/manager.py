    ...
    def _create_wrapped_model(
        ...
        **kwargs,
        ...
        if inspect.isclass(model_obj):
            # Class-based models
            return self._wrap_class_based_model(
                model_obj, framework, track_metrics, version, **kwargs
            )
        elif inspect.isfunction(model_obj) or inspect.ismethod(model_obj):
            # Function-based models
            return self._wrap_function_based_model(
                model_obj, framework, track_metrics, version, **kwargs
            )
        else:
            # Instance-based models
            return self._wrap_instance_based_model(
                model_obj, framework, track_metrics, version, **kwargs
            )
            ...
            def method_wrapper(self, *args, **kwargs):
                ...
                try:
                    ...
                    if method_name in ["predict", "evaluate"] and track_metrics:
                        self._track_metrics(
                            method_name, args, kwargs, result, track_metrics
                        )
        ...
        for method_name in methods_to_wrap:
            if hasattr(model_instance, method_name):
                ...
                def create_wrapper(original_method, method_name):
                    def method_wrapper(*args, **kwargs):
                        ...
                        try:
                            ...
                            elapsed = time.time() - start_time
                            logger.debug(
                                f"{method_name} completed in {elapsed:.4f} seconds"
                            )
                ...
                setattr(
                    model_instance,
                    method_name,
                    create_wrapper(original_method, method_name),
                )

        return model_instance

    def _register_model(
        self, model: Any, framework: str, track_metrics: Optional[List[str]] = None
    ) -> None:
