...
SDK-9 replaces this with the real scope-stack implementation (thread-local
state, parent pointers, max depth 64). For now the context manager is a
...
def scope(name: str, index: int | None = None, **attrs: Any) -> Iterator[None]:
    ...
    if not _warned:
        warnings.warn(
            "cirron.scope() runtime is not implemented yet (SDK-9); "
