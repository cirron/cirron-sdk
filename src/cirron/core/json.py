...
Two jobs, both of which keep one bad value from costing a whole flush tick.
Substitution: ``_safe_attrs`` / ``_json_safe`` coerce attr values that
``json.dumps`` would reject, and ``nonfinite_token`` / ``stats_to_wire``
replace non-finite floats, which Python would otherwise emit as the bare
``NaN`` / ``Infinity`` tokens that no conforming parser accepts. Encoding:
:func:`dumps` is the only place in the SDK that calls ``json.dumps`` on a
batch, and it falls back to a scrubbed retry rather than raising.

This module is a leaf on purpose. It imports nothing from ``cirron`` except
:mod:`cirron.core.swallow`, so the policy cannot live in ``transport.py`` or
``flush.py`` without creating an import cycle.
"""
...
def nonfinite_token(value: float) -> str | None:
    ...
    Args:
        value: The float (or float subclass) to classify.
...
def _finite_or_token(value: float) -> Any:
    ...
    :func:`nonfinite_token` collapses two different answers into ``None``:
    "this float is finite", and "this float could not be classified",
    meaning a subclass whose comparisons raise. The second case must not
    pass through. Handing an unclassifiable ``nan`` to the encoder raises
    the very ``ValueError`` that :func:`dumps`'s scrubbed retry is supposed
    to be immune to, and because the retry is the last attempt, that costs
    the batch.

    Args:
        value: The float (or float subclass) to screen.
...
def _json_safe(value: Any, _depth: int = 0, _seen: frozenset[int] = frozenset()) -> Any:
    ...
    Args:
        ...
        _seen: ``id()``s of the containers on the path from
...
def _rebuilt_attrs(attrs: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        attrs: Attrs dict adopted from the hot path.
...
def dumps(obj: Any, *, separators: tuple[str, str] | None = _COMPACT) -> str:
    ...
    When that raises, the object is deep-scrubbed through :func:`_json_safe`
    and re-encoded. The retry sees only cycle-free, depth-bounded JSON
    natives with string keys and provably finite floats, so the encoder has
    ...
    bug. It warns once per process and is counted for ``ci.health()``.

    Args:
        obj (Any): The object to serialize.
        separators: Passed through to ``json.dumps``. Defaults to compact
            ``(",", ":")``; pass ``None`` to keep the stdlib's spaced output.
...
def _warn_fallback(exc: BaseException) -> None:
    ...
    Args:
        exc: The ``json.dumps`` failure that triggered the
            fallback.
    """
